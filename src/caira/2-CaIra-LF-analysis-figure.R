...
set.seed(27313)
pdf("~/SLAbcurves/results/figs/caira-LF-curves.pdf",height=15,width=5)
lo <- layout(mat=matrix(1:6,nrow=3,ncol=2,byrow=TRUE),widths=c(1,0.2))

SLAb.plotEYax(wb123.fit1,wb123.fit3,expression(paste(italic('W. bancrofti'), " Wb123")),"A",ylabel=T)
SLAb.plotEYx(mu.wb123[,1],mu.wb123[,2],diff.wb123) 

SLAb.plotEYax(bm14.fit1,bm14.fit3,expression(paste(italic('W. bancrofti'), " Bm14")),"B",ylabel=T)
SLAb.plotEYx(mu.bm14[,1],mu.bm14[,2],diff.bm14) 

SLAb.plotEYax(bm33.fit1,bm33.fit3,expression(paste(italic('W. bancrofti'), " Bm33")),"C",ylabel=T,xlabel=T)
