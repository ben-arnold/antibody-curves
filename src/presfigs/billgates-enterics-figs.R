...
#
# make a summary plot for the handout
# for the may 3, 2016 dinner with bill gates
#
#--------------------------------------
...
pdf("~/dropbox/k01/presentations/160503-billgates/figs/enterics-usa-haiti.pdf",height=20,width=5)
lo <- layout(mat=matrix(1:8,nrow=4,ncol=2,byrow=TRUE),widths=c(1,0.2,1,0.2))
...
SLAb.plotEYax(usa.cp23,haiti.cp23,expression(paste(italic('Cryptosporidium parvum'), " Cp23")),"a",ylabel=T)
SLAb.plotEYx(EYx.usa.cp23,EYx.haiti.cp23,diff.cp23) 

SLAb.plotEYax(usa.giar,haiti.giar,expression(paste(italic('Giardia intestinalis'), " VSP-5")),"b",ylabel=T)
...
SLAb.plotEYax(usa.salb,haiti.salb,expression(paste(italic('Salmonella sp.'), " LPS Group B")),"c",ylabel=T)
...
SLAb.plotEYax(usa.norogii,haiti.norogii,"Norovirus GII.4 NO","d",ylabel=T,xlabel=T)
