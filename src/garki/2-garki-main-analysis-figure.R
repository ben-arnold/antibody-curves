...
plot(p.c12$pYframe$Age,p.c12$pYframe$pY,type="l",lwd=2,col=c.cols,
	...
	lines(p.tr1$pYframe$Age,p.tr1$pYframe$pY,col=i.cols[1])
	lines(p.tr2$pYframe$Age,p.tr2$pYframe$pY,col=i.cols[2])
	
	text(20,3.1,"Control",cex=1,adj=1,font=2,col=c.cols)
	text(20,3.9,"Intervention\n(survey 1, 2)",cex=1,adj=1,font=1,col=i.cols[1])
...
plot(p.c345$pYframe$Age,p.c345$pYframe$pY,type="l",lwd=2,col=c.cols,
	ylim=c(0,4),ylab="",yaxt="n",
	xlim=c(0,max(xtics)+1),xlab="",xaxt="n",
	bty="n",las=1
	)
	...
	# segments(x0=min(xtics),x1=max(xtics),y0=ytics,lty=2,col="gray70")
	axis(1,at=xtics,cex.axis=1.5)
	lines(p.tr3$pYframe$Age,p.tr3$pYframe$pY,col=i.cols[3])
	lines(p.tr4$pYframe$Age,p.tr4$pYframe$pY,col=i.cols[4])
	lines(p.tr5$pYframe$Age,p.tr5$pYframe$pY,col=i.cols[5])
...
op <- par(mar=c(4,5,7,1)+0.1)
plot(p.c78$pYframe$Age,p.c78$pYframe$pY,type="l",lwd=2,col=c.cols,
	ylim=c(0,4),ylab="",yaxt="n",
	xlim=c(0,max(xtics)+1),xlab="",xaxt="n",
	bty="n",las=1
	)
	...
	# segments(x0=min(xtics),x1=max(xtics),y0=ytics,lty=2,col="gray70")
	axis(1,at=xtics,cex.axis=1.5)
	lines(p.tr6$pYframe$Age,p.tr6$pYframe$pY,col=i.cols[6])
	lines(p.tr7$pYframe$Age,p.tr7$pYframe$pY,col=i.cols[7])
	lines(p.tr8$pYframe$Age,p.tr8$pYframe$pY,col=i.cols[8])
