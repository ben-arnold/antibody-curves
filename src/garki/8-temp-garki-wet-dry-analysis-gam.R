...
#-------------------------------
# 8-temp-garki-wet-dry-analysis-gam.R
#
...
# Pre-intervention period fitted curves
# stratified by season, averaged over control + intervention
p.c1 <-gam(log10(ifatpftitre+1)~lo(ageyrs),data=d.c1,family='gaussian')
p.c2 <-gam(log10(ifatpftitre+1)~lo(ageyrs),data=d.c2,family='gaussian')
p.c2 <-locfit(log10(ifatpftitre+1)~ageyrs,data=d.c2)
p.c3 <-gam(log10(ifatpftitre+1)~lo(ageyrs),data=d.c3,family='gaussian')
p.c4 <-gam(log10(ifatpftitre+1)~lo(ageyrs),data=d.c4,family='gaussian')
p.c5 <-gam(log10(ifatpftitre+1)~lo(ageyrs),data=d.c5,family='gaussian')

## predict new data
pp.c1 <-predict(p.c1,type='terms',newdata=d.c1[d.c1$ageyrs<=10,])

cbPalette <- c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")
cols <- cbPalette[2:8]
cols <- cols[c(2,1,3,6,5)]
plot(p.c1,rugplot=F,col=cols[1],xlim=c(0,10))
plot(p.c2,rugplot=F,col=cols[2])
plot(p.c3,rugplot=F,col=cols[3])
plot(p.c4,rugplot=F,col=cols[4])
plot(p.c5,rugplot=F,col=cols[5])
